...
def tail_points(opponent_score):
    ...
    return 2 * (tens - ones) + 1 if tens > ones else 2 * (ones - tens) + 1
    # END PROBLEM 2
...
def perfect_square(score):
    factor = 1
    while factor ** 2 < score:
        factor += 1
    return factor ** 2 == score
...
def max_scoring_num_rolls(dice=six_sided, total_samples=100000):
    ...
    max_score = 0
    for i in range(1,11):
        average_score = make_averaged(roll_dice,total_samples)(i, dice)
...
average_6 = make_averaged(roll_dice)(6)
average_1 = make_averaged(roll_dice)(1)
average_2 = make_averaged(roll_dice)(2)
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    def distance(score, goal = GOAL):
        return abs(score - goal)
    def greed_roll_num_and_points(score, opponent_score, threshold=12, num_rolls=6):
        num = square_strategy(score, opponent_score, threshold)
        if num == 0:
            points =  square_update(0, score, opponent_score) - score 
        else:
            points = average_6
        return num, points
    #0 for Player and 1 for Opponent
    greed0, greedPoint0 = greed_roll_num_and_points(score, opponent_score, average_6)
    greed1, greedPoint1 = greed_roll_num_and_points(opponent_score, score, average_6)
    if distance(opponent_score) > distance(score) and distance(opponent_score) + greedPoint1 > GOAL or\
        distance(opponent_score) < distance(score) and distance(score) + greedPoint0 < GOAL:
        decision = greed0 
    elif distance(opponent_score) + greedPoint1 < GOAL and distance(score) + greedPoint0 > GOAL:
        decision = 1 if distance(average_2) > distance(average_1) else 2
    else:
        decision = 6
