...
def has_path(t, word):
    ...
    if label(t) == word:
        return True
    return label(t) == word[0] and (True in [has_path(b, word[1:]) for b in branches(t)])
...
def mul_interval(x, y):
    ...
    p3 = lower_bound(y) * upper_bound(x)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    temp = interval(-lower_bound(y),-upper_bound(y))
    return add_interval(x, temp)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(x) * upper_bound(y) * lower_bound(x) *lower_bound(y) != 05+
