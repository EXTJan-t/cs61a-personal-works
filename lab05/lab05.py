...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_a = distance(temp_city, city_a)
    distance_b = distance(temp_city, city_b)
...
def count_palindromes(L):
    ...
    2
    """
    return len([w for w in L if w.lower() == w[::-1].lower()])
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    domain = [x for x in seq if fn(x) >= lower and fn(x) <= upper]
    func_range = [fn(x) for x in domain]
    result = []
    for i in range(len(domain)):
        result.append([domain[i], func_range[i]])
    return result
