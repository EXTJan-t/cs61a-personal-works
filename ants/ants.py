...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        location = self.place
        range_count = 1  #strange error pops out while testing this case, but doesn't affect usage
        while not location.is_hive:
            if location.bees and (self.lower_bound <= range_count <= self.upper_bound):
                return random_bee(location.bees)
            if range_count > self.upper_bound:
                break
...
class ScubaThrower(ThrowerAnt):
    """A throwant but waterproof is a lot more expensive"""
...
# END Problem 12
    ...
    def remove_from(self, place):
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        assert False, 'Cannot remove QueenAnt'
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        insects_distance_dic = {}
        location = self.place
        distance = 0
        while location:
            if location.bees:
                for b in location.bees:
                    insects_distance_dic[b] = distance
            distance += 1
            location = location.entrance
        return insects_distance_dic
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return self.damage - self.insects_shot * 0.0625 - distance * 0.25
        # END Problem Optional 2
